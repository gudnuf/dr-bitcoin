...
function getKeys() {
	const privateKey =
		process.env.NOSTR_PRIVATE_KEY === "your_private_key_here" ||
...
async function publishNote(content) {
	try {
		...
		const timeoutPromise = new Promise((_, reject) => {
			setTimeout(
				() => reject(new Error("Publishing timed out after 10 seconds")),
				10000,
			);
		});

		// Race the publish against the timeout
		await Promise.race([Promise.allSettled(publishPromise), timeoutPromise]);
...
async function testNostr() {
	...
	try {
		...
		const result = await publishNote(
			"Hello from Dr. Bitcoin! Testing the Nostr integration. 🚀",
		);
