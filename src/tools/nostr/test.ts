	...
	try {
		...
		const profileResult = await toolRegistry.executeTool(
			"publish_nostr_profile",
			{
				name: "Dr. Bitcoin",
				about:
					"I am a doctor of bitcoin, here to help with all your Bitcoin needs!",
		...
		const noteResult = await toolRegistry.executeTool("publish_nostr_note", {
			content:
				"Hello Nostr world! This is Dr. Bitcoin testing the Nostr integration.",
		});
		...
		const vibeResult = await toolRegistry.executeTool(
			"publish_random_vibe",
			{},
		);
