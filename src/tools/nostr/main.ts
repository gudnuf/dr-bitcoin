...
export const MANAGE_NOSTR_PROFILE: Tool = {
	...
	function: {
		...
		description:
			"Create, read, update, or delete a social media profile on nostr",
		parameters: JSON.stringify({
			...
			properties: {
				operation: {
					type: "string",
					description:
						"The operation to perform on the profile (create, read, update, delete)",
				...
				publicKey: {
					type: "string",
					description:
						"Optional public key for operations on profiles other than the default one",
				},
			},
			required: ["operation"],
		}),
...
export const NOSTR_BROWSE_FEED: Tool = {
	...
	function: {
		...
		description:
			"Browse a feed of short text notes that may also have threads or comments",
		parameters: JSON.stringify({
			type: "object",
			properties: {
				feed_type: {
				...
				pubkey: {
					type: "string",
					description:
						"Public key of user to view posts from (required for 'user' feed type)",
				},
				hashtag: {
					type: "string",
					description:
						"Hashtag to filter by (required for 'hashtag' feed type, without the # symbol)",
				},
				limit: {
					type: "integer",
...
export const NOSTR_VIEW_THREAD: Tool = {
	...
	function: {
		...
		description:
			"View a complete thread including the original post and all replies",
		parameters: JSON.stringify({
			type: "object",
			properties: {
				note_id: {
					type: "string",
					description:
						"ID of the note/post to view the thread of (hex or note1...)",
				},
				limit: {
					type: "integer",
		...
		parameters: JSON.stringify({
			...
			properties: {
				...
				root: {
					type: "string",
					description:
						"ID of the root note in the thread if different from reply_to (hex or note1...)",
				},
			},
...
export async function publishNostrProfile(args: any) {
	console.log(
		chalk.cyan("📝 Publishing Nostr Profile:"),
...
export async function publishNostrEvent(args: any) {
	console.log(
		chalk.cyan("📤 Publishing Nostr Event:"),
		chalk.gray(JSON.stringify(args, null, 2)),
	);
...
export async function publishNostrNote(args: any) {
	console.log(
		chalk.cyan("📝 Publishing Nostr Note:"),
		chalk.gray(JSON.stringify(args, null, 2)),
	);
