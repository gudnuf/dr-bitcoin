...
export class AutonomousAgent extends AgentWorkflow {
  ...
  public start(): void {
    ...
    if (!lifeGoal) {
      throw new Error('Life goal must be set before starting the autonomous agent');
    }
    ...
    this.log(`Running on ${chalk.cyan(this.formatInterval(this.agentConfig.runInterval))} intervals`);
  ...
  public stop(): void {
    ...
    this.log(`${chalk.yellow('Autonomous agent stopped')} after ${this.cycleCount} cycles`);
  }
  ...
  private async runCycle(): Promise<void> {
    try {
      ...
      this.log(`${chalk.cyan('Starting cycle')} #${this.cycleCount} | Current objectives: ${objectiveCount} | Completed: ${completedCount}`);
      ...
      if (toolCalls && toolCalls.length > 0) {
        this.log(`Executing ${chalk.green(toolCalls.length.toString())} tools...`);
        const followUpMessage = await this.handleToolCalls(toolCalls);
        
        const truncatedFollowUp = followUpMessage.length > 150 
      ...
      this.log(`${chalk.green('Cycle completed')} #${this.cycleCount} | Next cycle in ${this.formatInterval(this.agentConfig.runInterval)}`);
  ...
  private scheduleNextCycle(): void {
    ...
    this.intervalId = setTimeout(() => this.runCycle(), this.agentConfig.runInterval) as unknown as NodeJS.Timeout;
  }
  ...
  private checkForObjectiveUpdates(message: string, memory: any): void {
    ...
    for (const phrase of newObjectivePhrases) {
      if (message.toLowerCase().includes(phrase)) {
        const startIndex = message.toLowerCase().indexOf(phrase) + phrase.length;
        let endIndex = message.indexOf('.', startIndex);
        if (endIndex === -1) endIndex = message.indexOf('\n', startIndex);
        if (endIndex === -1) endIndex = message.length;
        
        const possibleObjective = message.substring(startIndex, endIndex).trim().replace(/^[:"'-\s]+|[:"'-\s]+$/g, '');
        
        if (possibleObjective.length > 10 && possibleObjective.length < 200) {
          this.setObjective(possibleObjective);
          this.log(`${chalk.green('Detected new objective:')} ${possibleObjective}`);
        }
      }
    }
  }
  ...
  private printStatusSummary(): void {
    ...
    const nextRunIn = this.intervalId 
      ? Math.max(0, this.agentConfig.runInterval - (timeSinceLastRun ? timeSinceLastRun * 1000 : 0))
      : null;
    
    console.log('\n' + chalk.cyan('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
    console.log(chalk.cyan('📊 AGENT STATUS SUMMARY'));
    console.log(chalk.cyan('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
    console.log(`${chalk.yellow('Running:')} ${this.isRunning ? chalk.green('Yes') : chalk.red('No')}`);
    console.log(`${chalk.yellow('Cycles completed:')} ${this.cycleCount}`);
    console.log(`${chalk.yellow('Current objectives:')} ${memory.goals.currentObjectives.length}`);
    console.log(`${chalk.yellow('Completed objectives:')} ${memory.goals.completedObjectives.length}`);
    
    if (lastRun) {
      console.log(`${chalk.yellow('Last run:')} ${lastRun.toLocaleString()} (${timeSinceLastRun}s ago)`);
    } else {
      console.log(`${chalk.yellow('Last run:')} Never`);
    }
    
    if (nextRunIn !== null) {
      console.log(`${chalk.yellow('Next run in:')} ${this.formatInterval(nextRunIn)}`);
    }
    ...
    if (memory.goals.completedObjectives.length > 0 && memory.goals.completedObjectives.length <= 5) {
      console.log(chalk.yellow('\nRecently completed objectives:'));
      memory.goals.completedObjectives.slice(-5).forEach((obj: string, i: number) => {
  ...
  private generateSelfPrompt(memory: any): string {
    ...
    const recentHistory = history.slice(-5).map((entry: any) => 
...
`;
  ...
  public async processInput(input: string): Promise<void> {
    try {
      this.log(`${chalk.blue('Processing user input')}: ${input.substring(0, 50)}${input.length > 50 ? '...' : ''}`);
      
      const { message, toolCalls } = await this.processUserInput(input);
      
      this.log(`Response: ${chalk.white(message.substring(0, 100))}${message.length > 100 ? '...' : ''}`);
      if (this.agentConfig.verbose) {
      ...
      // Handle any tool calls
      if (toolCalls && toolCalls.length > 0) {
        this.log(`Executing ${chalk.green(toolCalls.length.toString())} tools from user input...`);
        const followUpMessage = await this.handleToolCalls(toolCalls);
        this.log(`Tool execution result: ${chalk.white(followUpMessage.substring(0, 100))}${followUpMessage.length > 100 ? '...' : ''}`);
      }
