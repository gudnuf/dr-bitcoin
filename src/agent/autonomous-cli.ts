...
export class AutonomousCLI {
  ...
  private async runCommandLoop(): Promise<void> {
    const welcomeMessage = boxen(
      ...
      chalk.green("• goal <text>") + " - Set the agent's life goal\n" +
      chalk.green("• start") + " - Start the autonomous agent\n" +
      chalk.green("• stop") + " - Stop the autonomous agent\n" +
      chalk.green("• status") + " - Check agent status\n" +
      chalk.green("• objective add <text>") + " - Add a new objective\n" +
      chalk.green("• objective complete <text>") + " - Mark an objective as complete\n" +
      chalk.green("• talk <text>") + " - Send a message to the agent\n" +
      chalk.green("• exit") + " - Quit the CLI",
      {
        padding: 1,
        margin: 1,
        borderStyle: "round",
      ...
      switch(command.toLowerCase()) {
        ...
        case "status":
          ...
          const statusBox = boxen(
            ...
            chalk.yellow("Running: ") + (status.isRunning ? chalk.green("Yes") : chalk.red("No")) + "\n" +
            chalk.yellow("Life Goal: ") + chalk.white(status.memory.goals.lifeGoal || "Not set") + "\n\n" +
            chalk.yellow("Current Objectives:") + "\n" +
            (status.memory.goals.currentObjectives.length > 0 
              ? status.memory.goals.currentObjectives.map((obj: string) => 
                  chalk.white(`- ${obj}`)
                ).join("\n")
              : chalk.gray("No current objectives")) + "\n\n" +
            chalk.yellow("Completed Objectives:") + "\n" +
            (status.memory.goals.completedObjectives.length > 0
              ? status.memory.goals.completedObjectives.map((obj: string) => 
                  chalk.gray(`- ${obj}`)
                ).join("\n")
              : chalk.gray("No completed objectives")) + "\n\n" +
            chalk.yellow("Last Run: ") + 
            chalk.white(status.memory.lastRun ? new Date(status.memory.lastRun).toLocaleString() : "Never"),
            {
              padding: 1,
              margin: 1,
              borderStyle: "round",
        ...
        case "objective":
          if (args.length < 2) {
            console.log(chalk.red("Invalid objective command. Use 'objective add <text>' or 'objective complete <text>'"));
            break;
          }
        ...
        case "talk":
          if (args.length === 0) {
            console.log(chalk.red("Please provide a message to send to the agent"));
            break;
          }
